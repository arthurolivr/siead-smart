# chat_app.py (Versão Final com Google Gemini)
...
st.title("🤖 Agente Financeiro Conversacional")
st.caption("Inteligência por Google Gemini. Tente 'qual a previsão para amanhã?'")

try:
    google_api_key = st.secrets["GOOGLE_API_KEY"]
except (FileNotFoundError, KeyError):
    st.error("ERRO: Chave de API da Google não encontrada. Crie a pasta .streamlit e o arquivo secrets.toml, e adicione sua GOOGLE_API_KEY nele.")
    st.stop()

@tool
def prever_faturamento_data_especifica(data: str) -> str:
    """
    Use esta ferramenta para prever o faturamento de um único dia específico.
    A data DEVE ser uma string no formato 'AAAA-MM-DD'.
    """
...
def resumir_previsao_periodo(dias: int) -> str:
    """
    Use esta ferramenta para obter um resumo da previsão de faturamento para um período futuro em dias.
    Use-a quando o usuário perguntar sobre 'próximos X dias', 'próximo mês' (use 30 dias), 'próximo ano' (use 365 dias), etc.
    """
...
def prever_faturamento_amanha() -> str:
    ...
    Esta ferramenta não precisa de argumentos.
    """
...
prompt = ChatPromptTemplate.from_messages([
    ("system", "Você é um assistente financeiro amigável e prestativo. Use as ferramentas disponíveis para responder às perguntas do usuário. Seja sempre conciso."),
